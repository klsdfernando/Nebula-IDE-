...
export class ChatSetupContribution extends Disposable implements IWorkbenchContribution {
	...
	private registerActions(context: ChatEntitlementContext, requests: ChatEntitlementRequests, controller: Lazy<ChatSetupController>): void {
		...
		class ChatSetupTriggerForceSignInDialogAction extends Action2 {
			...
			constructor() {
				super({
					...
					title: localize2('forceSignIn', "Sign in to Superluminal")
				});
			}

			override async run(accessor: ServicesAccessor): Promise<unknown> {
				return superluminalSignIn(accessor, context);
			}
		}
		...
		class ChatSetupFromAccountsAction extends Action2 {
			...
			constructor() {
				super({
					...
					title: localize2('triggerChatSetupFromAccounts', "Sign in to Superluminal..."),
					menu: {
						id: MenuId.AccountsContext,
						group: '2_superluminal',
		...
		// ─── Superluminal Sign-In Helper ───
		async function superluminalSignIn(accessor: ServicesAccessor, entitlementContext: ChatEntitlementContext): Promise<void> {
			...
			// Open the Superluminal login page in browser
			openerService.open(URI.parse('http://localhost:3500/auth/login'));

			// Show input box for token
			const token = await quickInputService.input({
				title: 'Superluminal – Paste Authentication Token',
				prompt: 'Sign in on the browser page that just opened, copy the token, and paste it here.',
			...
			try {
				...
				if (response.res.statusCode !== 200) {
					notificationService.error('Superluminal: Invalid or expired token. Please try signing in again.');
					return;
				}

				// Store the token persistently
				storageService.store('superluminal.auth.token', token.trim(), StorageScope.PROFILE, StorageTarget.MACHINE);
				...
				try {
					...
					if (text) {
						...
						storageService.store('superluminal.auth.user', JSON.stringify(data.user), StorageScope.PROFILE, StorageTarget.MACHINE);
					}
				} catch { /* ignore parse errors */ }

				// Update the entitlement context to mark user as signed in
				await entitlementContext.update({
					entitlement: 5, // ChatEntitlement.Free
					organisations: undefined,
					sku: 'superluminal_user',
					copilotTrackingId: undefined,
				});

				notificationService.info(`✅ Superluminal: Welcome, ${userName}! You are now signed in.`);
			} catch (err) {
				notificationService.error('Superluminal: Could not validate token. Is the backend server running on localhost:3500?');
			}
		}
